  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-row items-center">
        {strategies.length > 0 && (
          <div className="rounded-lg px-2 py-5 shadow my-5 mx-2 bg-primary-bg">Money received</div>
        )}
        {strategies.length > 0 && <div>➡️</div>}
        <div className="flex flex-col">
          {strategies.map((strategy, index) => {
            return (
              <div key={index} className="mx-2 my-1 rounded-lg px-5 py-5 shadow bg-gray-200">
                <div className="flex flex-row items-center">
                  <div className="text-neutral-700 rounded-lg shadow bg-orange-300 px-5 py-5 mr-5">{`${simpleShortBalance(
                    '' + (1 / strategies.length) * 100,
                    4
                  )}%`}</div>
                  <div className="flex flex-col">
                    <Select
                      id="alert-type"
                      value={strategy.actionType || 'DEFAULT'}
                      onChange={(event) => {
                        if (event.target.value !== 'DEFAULT') {
                          setStrategies(
                            strategies.map((strat, strategyIndex) => {
                              if (strategyIndex === index) {
                                return {
                                  ...strat,
                                  actionType: event.target.value as MoneyStrategyType,
                                };
                              }
                              return strat;
                            })
                    ...
                    >
                      ...
                      <MenuItem value={MoneyStrategyType.TRANSFER_TO_ADDRESS}>
                    ...
                    {(strategy.actionType === MoneyStrategyType.TRANSFER_TO_ADDRESS ||
                      strategy.actionType === MoneyStrategyType.SWAP_ON_UNISWAP) && (
                      <TextField
                    ...
                    {(strategy.actionType === MoneyStrategyType.SWAP_ON_UNISWAP ||
                      strategy.actionType === MoneyStrategyType.DEPOSIT_TO_AAVE) && (
                      <TextField
