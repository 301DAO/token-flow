...
contract Proxy is IProxy, Storage, Config {
  ...
  function _exec(
    ...
    bool success;
    assembly {
      success := delegatecall(sub(gas(), 5000), to_, add(data_, 0x20), mload(data_), 0, 0)
    ...
    if (!success) {
      ...
      if (counter_ == type(uint256).max) {
        ...
        revert(string(abi.encodePacked(counter_.toString(), '_', abi.decode(result, (string)))));
      }
    }
  }
  ...
  function _calFee(uint256 amount, uint256 feeRate) internal pure returns (uint256) {
