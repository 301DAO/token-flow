...
contract HAaveProtocolV3 is HandlerBase, IFlashLoanReceiver {
  ...
  function withdrawETH(uint256 amount) external payable returns (uint256 withdrawAmount) {
  ...
  function _supply(address asset, uint256 amount) internal {
    ...
    try IPool(pool).supply(asset, amount, address(this), REFERRAL_CODE) {} catch Error(
  ...
  function _borrow(
    ...
    try IPool(pool).borrow(asset, amount, rateMode, REFERRAL_CODE, onBehalfOf) {} catch Error(
      string memory reason
    ) {
  ...
  function _repay(
    ...
    try IPool(pool).repay(asset, amount, rateMode, onBehalfOf) {} catch Error(
      string memory reason
    ) {
