...
interface IFeeRuleRegistry {
  ...
  /* View Functions */
  function calFeeRateMulti(address usr, uint256[] calldata ruleIndexes)
    external
    view
  ...
  function calFeeRateMultiWithoutBasis(address usr, uint256[] calldata ruleIndexes)
    external
    view
    returns (uint256 scaledRate);

  function calFeeRateWithoutBasis(address usr, uint256 ruleIndex)
    external
    view
    returns (uint256 scaledRate);
}
